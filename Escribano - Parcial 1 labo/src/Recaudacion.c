...
int altaRecaudacion(Recaudacion recauda[],int* nextIdRec, int size,Tipo tipos[],int sizeT,Contribuyente lista[], int sizeC)
{
	...
	if(recauda!=NULL && SIZE>0 && tipos!= NULL && SIZET>0)
	{
		...
		else
		{
			...
			printRecaudacion(recauda[position],SIZER,tipos,SIZET);
		}
		ret=0;
	}
	return ret;
		...
		if(lista!=NULL && SIZE>0)
		  {
			...
			get_Entero(&auxId,1,"\nIngrese id a refinanciar ","\n Error: reingrese id a refinanciar: ",1,9999);
		    		        ...
		    		        {
		    do{
		    	auxIdC=recauda[position].idContribuyente;
		    				    		  if(findContribuyenteById(lista,SIZE, auxIdC, &pos)==-1){
		    				    			  printf("ID inexistente\n");
		    				    		  }else{
		    				    			  printContribuyente(lista[pos],SIZE);
		    				    		  }
		    	{
			...
			if(lista!=NULL && SIZE>0)
			  {
			    ...
			    do{
			    	...
			    	printf("Desea volver a saldar? s/n : \n");
			    				        fflush(stdin);
			    				        scanf("%c", &continuar);
			    }while(continuar=='s');
			    		        }
	...
	if(recauda!= NULL && SIZE >=0)
	    {
	        for(i=0;i<SIZE;i++)
	        {
	            if(recauda[i].isEmpty==1)
	                continue;
	            else if(recauda[i].idContribuyente==valor)
	            {
	                retorno=0;
	                *position=i;
	                break;
...
for(int i=0;i<SIZER;i++)
{
	if(strcmp(recauda[i].status,"Saldar")==0 && recauda[i].isEmpty==0)
	{
		...
		if(findContribuyenteById(lista,SIZE,auxId, &position)==0)
		{
			...
			printf("***************************Lista de Recaudaciones***************************\n");
			printf("%d    %d   %s     %d     %s %d %s %s %d\n",recauda[i].idRec, recauda[i].mes ,descripcion, recauda[i].importe,recauda[i].status,
					recauda[position].idContribuyente,
					buffNombre,buffApellido,lista[position].cuil);
